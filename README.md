...
| 显示屏 | 2.8吋触摸屏（FT5206/GT9xxx） | LVGL 图形交互、数据展示 |
| Wi-Fi 模块 | ESP8266（AT 固件） | 联网、OTA 固件下载 |
| RS485 收发器 | SP3485 | 电平转换，Modbus 总线通信 |
| 外置 Flash | W25Q128 | OTA 固件存储 |
| EEPROM | 24C02 | 参数与配置存储 |
| 调试器 | ST-Link / DAP | 程序下载与在线调试 |

> 具体引脚分配见各 BSP 源码（`Drivers/BSP/*`）与 `User/main.c` 初始化流程。
